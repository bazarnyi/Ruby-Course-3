...
module Helpers
  ...
  def new_project(user)
    visit 'https://gitlab.testautomate.me/projects/new#blank_project'

    # find('[data-qa-selector="project_name"]').click
    # type('#new_user_last_name').set user.lastname
    # find('[data-qa-selector="project_create_button"]').click

    @new_project_page = NewProjectPage.new
    @new_project_page.load
    @new_project_page.find('[data-qa-selector="project_name"]').set "#{@lastname}Project"
    @new_project_page.find('[data-qa-selector="project_create_button"]').click
  end
end
